...
function onScriptUpdate()
	...
	if math.min(currentSpeed * (turnSpeed + (1 - turnSpeed)) + A3, topSpeed) == topSpeed then 
		if math.ceil(((1 - ((topSpeed - A3) / topSpeed))/(1 - turnSpeed)) * 7) == 2 then SetMainStickX(167)
		else SetMainStickX(152)
		end
	else
		if math.ceil(((1 - ((topSpeed - A3) / topSpeed))/(1 - turnSpeed)) * 7) == 2 then SetMainStickX(152)
